            ...
            <div class="col">
                <div class="card">
                    <h3 class="card-header d-flex justify-content-between align-items-center">
                        Data Petugas
                        <button class="btn btn-icon btn-primary" type="button" v-b-modal.modalPetugas v-on:click="Add">
                    ...
                    <div class="table-responsive">
                        <b-table striped hover :items="petugas" :fields="fields" class="table align-items-center table-flush">
                            <template v-slot:cell(level)="data">
                                <b-badge variant="primary">{{ data.item.level }}</b-badge>
                            </template>
                            <template v-slot:cell(Aksi)="data">
                                <b-button size="sm" v-on:click="Edit(data.item.id)" v-b-modal.modalEdit class="btn btn-outline-primary">Ubah</b-button>&nbsp;
...
<script>
    module.exports = {
        data : function(){
            ...
            token: "",
            petugas: [],
            fields: ["id", "nama", "username", "telp", "level", "Aksi","nik"],
            }
        },
        ...
        methods: {
            ...
            Edit : function(id){
                let conf = { headers: { "Authorization" : 'Bearer ' + this.token } };
                this.axios.get("/petugas/" + id, conf)
                .then(response => {
                    if(response.data.success == true){
                        this.action = "update";
